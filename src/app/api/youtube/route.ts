  ...
  try {
    switch (action) {
      ...
      case 'recent-videos':
        ...
        try {
          // First try to get liked videos (these have full video details)
          ...
          // If no liked videos, try to get search history
          const searchHistoryResponse = await fetch(
            `https://www.googleapis.com/youtube/v3/search?part=snippet&forMine=true&type=video&maxResults=20&key=${YOUTUBE_API_KEY}`,
            {
              headers: {
                'Authorization': `Bearer ${accessToken}`,
              },
          ...
          if (searchHistoryData.items && searchHistoryData.items.length > 0) {
            // Get detailed video information for search results
            const videoIds = searchHistoryData.items.map((item: any) => item.id.videoId);
            ...
            return NextResponse.json(videoDetailsData);
          }

          // Fallback: return empty array if no videos found
      ...
      case 'top-channels':
        ...
        try {
          ...
          do {
            const url = `https://www.googleapis.com/youtube/v3/subscriptions?part=snippet&mine=true&maxResults=50&key=${YOUTUBE_API_KEY}${nextPageToken ? `&pageToken=${nextPageToken}` : ''}`;
            
            const subscriptionsResponse = await fetch(url, {
              headers: {
            ...
            if (subscriptionsData.items) {
              allSubscriptions = allSubscriptions.concat(subscriptionsData.items);
            }
            
            nextPageToken = subscriptionsData.nextPageToken || null;
            
            // Limit to first 100 subscriptions to avoid too many API calls
            if (allSubscriptions.length >= 100) {
              break;
            }
            
          } while (nextPageToken);
          
          return NextResponse.json({ items: allSubscriptions });
